...
# target_border: float, [default=None] Border for target binarization
######################################################################
...
cat_paramets = {"learning_rate" : (0.01,0.3),
                'depth' : (8,10),
                'od_pval' :(0.4,0.6),
                'model_size_reg' : (0.4,0.5),
                'l2_leaf_reg' :(4,6),
                # 'fold_permutation_block':(7,9),
                'target_border':(1,10)
                }
...
def xgb_hamsu(learning_rate,depth,od_pval,model_size_reg,l2_leaf_reg,target_border
    ...
    params = {
        ...
        'depth' : int(round(depth,0)),  #무조건 정수
        'l2_leaf_reg' : int(round(l2_leaf_reg,0)),
        'model_size_reg' : max(min(model_size_reg,1),0), # 0~1 사이의 값이 들어가도록 한다.
        'target_border' : max(min(model_size_reg,1),0), # 0~1 사이의 값이 들어가도록 한다.
