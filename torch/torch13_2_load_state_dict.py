...
model2.load_state_dict(torch.load(path + 'torch13_1_state_dict.pt'))
# save 와 load의 코드가 상이하므로 주의할 것
# save는 데이터부터 평가,예측까지 모든 구성으로 최적의 weight와 최소의 loss로 
# weight를 저장한다.
# load는 torch.load라는 method로 불러오고 
############### 하단 내용 붙인 것##################
