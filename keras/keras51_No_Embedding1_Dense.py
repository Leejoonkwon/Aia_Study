...
from tensorflow.python.keras.layers import LSTM,Dense,Embedding,Flatten,Conv1D
...
# Embedding을 하지 않지만 DNN으로 할 경우에는 reshape를 통해 차원을 바꿔줄 필요는 없지만 성능이 더 좋지는 않다.
model = Sequential() # 인풋은 (14,5)가 31개
...
# model.add(LSTM(32))
# model.add(Conv1D(32,2)) #stride: kernel을 얼마만큼씩 이동하면서 적용할 것인가 (Default: 1) -> 커널사이즈보다 크게 
# model.add(Flatten())
