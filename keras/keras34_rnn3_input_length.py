...
model.add(SimpleRNN(units=100, input_length=3,input_dim=1)) #위와 같은 개념
model.add(SimpleRNN(units=100,input_dim=1, input_length=3)) #순서가 바뀌어도 상관없지만 가독성을 위해 length를 먼저

#input_shape=[batch, timesteps (자르는 단위)param에서는 무시 , feature(열=input_dim)].
# model.add(SimpleRNN(unit,return_sequence=False, input_shape=(3,1),activation='swish'))
